...
```mermaid
flowchart TD
    subgraph Problem["Decode 阶段特点"]
        P1["每次只生成 1 个 token"]
        P2["计算量小 - 相对于 Prefill"]
        P3["Kernel launch 开销占比大"]
    end

    subgraph Traditional["传统执行"]
        direction LR
        T1["CPU: launch kernel 1"] --> T2["CPU: launch kernel 2"] --> T3["CPU: launch kernel 3"]
        T4["GPU: compute 1"] --> T5["GPU: compute 2"] --> T6["GPU: compute 3"]
        T1 -.->|"launch 开销"| T4
        T2 -.->|"launch 开销"| T5
        T3 -.->|"launch 开销"| T6
    end

    subgraph CUDAGraph["CUDA Graph 执行"]
        direction LR
        C1["CPU: replay graph - 单次提交"] --> C2["CPU: 空闲"]
        C3["GPU: compute 1 -> 2 -> 3 连续执行"]
        C1 -.->|"单次 launch"| C3
    end

    Problem --> Traditional
    Traditional -->|"优化为"| CUDAGraph
```
...
```mermaid
flowchart TD
    A["ModelRunner.initialize"] --> B["1: init_torch_distributed()<br/>初始化 TP/PP 进程组<br/>设置 NCCL 通信"]
    B --> C["2: load_model()<br/>加载模型权重<br/>应用量化 - 如果启用"]
    C --> D["3: init_attention_backend()<br/>选择 FlashInfer/FlashAttention/Triton 后端"]
    D --> E["4: init_lora_manager()<br/>如果启用"]
    E --> F["5: kernel_warmup()<br/>FlashInfer autotune"]
    F --> G["6: init_device_graphs()<br/>创建 CudaGraphRunner<br/>捕获所有 batch size"]
    G --> H["7: init_piecewise_cuda_graphs()<br/>如果启用"]
```
