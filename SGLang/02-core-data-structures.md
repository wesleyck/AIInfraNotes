...
flowchart TB
    ...
    subgraph Worker["Worker 层"]
        MWB["ModelWorkerBatch<br/>解耦调度与执行的纯数据容器"]
    end
...
| 包含 Req | 是 (`reqs: List[Req]`) | 否 (裁剪不可序列化对象) |
| 内存池引用 | 是 | 否 |
| 缓存引用 | 是 (`tree_cache`) | 否 |
...
> 而 Worker 和 Runner 持有独立的 ModelWorkerBatch 异步执行，两者互不干扰。
...
sequenceDiagram
    ...
    User->>TM: 发送请求 (text, image)
    TM->>TM: Tokenize, Process Image
    Note right of TM: → TokenizedGenerateReqInput<br/>(input_ids, mm_inputs)
    TM->>Sched: GenerateReqInput

    rect rgb(240, 248, 255)
        Note over Sched: ① Req 创建
        Sched->>Sched: Req(rid, origin_input_ids, sampling_params)
        Sched->>Sched: waiting_queue.append(req)
    end

    rect rgb(245, 255, 245)
        Note over Sched: ② ScheduleBatch 构建
        Sched->>Pool: alloc_req_slot() → req_pool_indices
        Sched->>Sched: ScheduleBatch.init_new([req])
        Sched->>Pool: alloc_for_extend() → out_cache_loc
        Sched->>Sched: batch.prepare_for_extend()
        Note right of Sched: ScheduleBatch 持有:<br/>reqs[], req_to_token_pool,<br/>token_to_kv_pool_allocator, tree_cache
    end

    rect rgb(255, 248, 240)
        Note over Sched,Worker: ③ ModelWorkerBatch (解耦)
        Sched->>Worker: batch.get_model_worker_batch()
        Note right of Worker: ModelWorkerBatch 仅含:<br/>input_ids, seq_lens, out_cache_loc,<br/>sampling_info, mm_inputs<br/>(不含 Req 对象/内存池引用)
    end

    rect rgb(248, 240, 255)
        Note over Worker,Runner: ④ ForwardBatch (GPU 化)
        Worker->>Runner: ForwardBatch.init_new(worker_batch, model_runner)
        Note right of Runner: ForwardBatch 额外添加:<br/>positions, attn_backend,<br/>req_to_token_pool, token_to_kv_pool<br/>(来自 model_runner 全局引用)
        Runner->>Runner: model.forward(forward_batch)
        Runner-->>Worker: logits / next_token
    end

    Worker-->>Sched: GenerationBatchResult
```
...
sequenceDiagram
    ...
    loop 直到所有请求完成
        rect rgb(245, 255, 245)
            Note over Sched: ① ScheduleBatch 更新
            Sched->>Pool: alloc_for_decode() → 每个 req 分配 1 个 KV slot
            Sched->>Sched: running_batch.prepare_for_decode()
            Note right of Sched: input_ids = [last_token_per_req]<br/>seq_lens += 1<br/>forward_mode = DECODE
        end

        rect rgb(255, 248, 240)
            Note over Sched,Worker: ② ModelWorkerBatch (解耦)
            Sched->>Worker: running_batch.get_model_worker_batch()
        end

        rect rgb(248, 240, 255)
            Note over Worker,Runner: ③ ForwardBatch → 前向计算
            Worker->>Runner: ForwardBatch.init_new(worker_batch, model_runner)
            Runner->>Runner: model.forward(forward_batch)
            Runner-->>Worker: next_token_ids
        end
        ...
        rect rgb(255, 245, 245)
            Note over Sched: ④ 后处理
            Sched->>Sched: req.output_ids.append(token)
            Sched->>Sched: check_finished()
            opt 请求完成
                Sched->>Pool: free_req_slot()
                Sched->>Sched: tree_cache.cache_finished_req(req)
            end
        end
    end
```
